...
Class Action {
	...
	function save_student(){
		...
		foreach($_POST as $k=> $v){
			...
			if(empty($LRN_NO)){
				...
				while($i == 1){
					$rand = mt_rand(1,99999999);
					$rand =sprintf("%'08d",$rand);
					$chk = $this->db->query("SELECT * FROM student_info where LRN_NO = '$rand' ")->num_rows;
					if($chk <= 0){
						$data .= ", LRN_NO='$rand' ";
						$i = 0;
					}
		...
		if(empty($id)){
			if(!empty($LRN_NO)){
				$chk = $this->db->query("SELECT * FROM students_info where LRN_NO = '$LRN_NO' ")->num_rows;
				if($chk > 0){
					return 2;
					exit;
				}
			}
			$save = $this->db->query("INSERT INTO student_info set $data ");
		}else{
			if(!empty($LRN_NO)){
				$chk = $this->db->query("SELECT * FROM student_info where LRN_NO = '$LRN_NO' and STUDENT_ID != $id ")->num_rows;
				if($chk > 0){
					return 2;
					exit;
				}
			}
			$save = $this->db->query("UPDATE student_info set $data where STUDENT_ID=".$id);
		}
		if($save)
			return 1;
	}
